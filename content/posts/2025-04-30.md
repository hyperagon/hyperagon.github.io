...
But funily enough it stumbled on this part of the code:
```
import sys
...
progress_bar.close()
```
Which shows several progress bars. one per line, instead of a single one like (suggested to see if *TQDM* was working):
```
import sys
from tqdm import tqdm
import time
...
Which reinforces my belief that by itself, AI is fast but mot very smart.
